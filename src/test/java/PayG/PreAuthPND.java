...
import Pages.PayG.GetTheParking;
import Pages.PayG.Login;
import Pages.PayG.PNDsignup;
import Pages.PayG.SavePayment;
import Pages.PayG.authorizeCard;
import Pages.PayG.dbVerification;
import Pages.PayG.stopPNDsession;
import Pages.PayG.vaidations;
import PayG.ObjectMapper.*;
import TestNGListeners.ApcoaListeners;
import junit.framework.Assert;

public class PreAuthPND {
	...
	@DataProvider
	public PayG.ObjectMapper.UrlMapper[] getEntryUrlData() throws Exception{
		String excelfilepath =System.getProperty("user.dir") + "/src/test/java/resources/PayG/Dataset/PreUthGMPPND.xlsx";
		...
		PayG.ObjectMapper.UrlMapper []url = new UrlMapper[1];
...
//		AutomationConfiguration.Driver.manage().timeouts().implicitlyWait(15, TimeUnit.SECONDS);
	...
	public void signup() throws InterruptedException {
		PNDsignup signup = PageFactory.initElements(CreateSession.getAutomationConfiguration().Driver, PNDsignup.class);
		Thread.sleep(5000);
		signup.signupPND();
	}
	@Test(priority = 3)
	...
	@Test(priority = 6)
	public void fetchSessionID() throws InterruptedException {
		vaidations fetch = PageFactory.initElements(CreateSession.getAutomationConfiguration().Driver, vaidations.class);
		Thread.sleep(3000);
		SessionId=fetch.fetchSessionId();
	...
	@Test(priority=6)
	public void parkingName() throws InterruptedException, IOException {
		Thread.sleep(5000);
		GetTheParking parking_name = PageFactory.initElements(CreateSession.getAutomationConfiguration().Driver, GetTheParking.class);	
		parkingName = parking_name.getTheParking();
		Thread.sleep(2000);}	
	@Test(priority=8)
	public void stopSessionPND() throws InterruptedException, IOException {
		Thread.sleep(5000);
		stopPNDsession stop = PageFactory.initElements(CreateSession.getAutomationConfiguration().Driver, stopPNDsession.class);	
		stop.stopPND(SessionId,parkingName,Promoname);
		Thread.sleep(10000);
	}
	@Test(priority=8)
