...
import Pages.Dashboard.MultiaccountPage;
...
public class Multiaccount {
	...
	public LoginMapper[] getLoginData() throws Exception{
		String excelfilepath = System.getProperty("user.dir") + "/src/test/java/resources/Multiaccount.xlsx";
	...
	@DataProvider
	public multiaccountMapper[] getmultiaccData() throws Exception{
		String excelfilepath = System.getProperty("user.dir") + "/src/test/java/resources/Multiaccount.xlsx";
		ExcelDriven.readExcelFile(excelfilepath,CreateSession.getAutomationConfiguration().Environment);
		String data = ExcelDriven.readDataRowandColumn(CreateSession.getAutomationConfiguration().Environment,CreateSession.getAutomationConfiguration().Runner,"multiaccount");
		ObjectMapper mapper = new ObjectMapper();
		multiaccountMapper []multiaccdata = new multiaccountMapper[1];
		multiaccdata[0] = mapper.readValue(data, multiaccountMapper.class);
		return multiaccdata;
	}
	...
	@Test(priority=3, dataProvider="getmultiaccData")
	public void multiaccount(multiaccountMapper MultiaccountMapper) throws InterruptedException{
		MultiaccountPage mp = PageFactory.initElements(CreateSession.getAutomationConfiguration().Driver, MultiaccountPage.class);
		mp.Multiacc(MultiaccountMapper);
	}
	
	@Test(priority=4)                   // scroll of the parking to select the desired pakring from the dropdown
	public void ScrollParking() throws InterruptedException{
		MultiaccountPage pp = PageFactory.initElements(CreateSession.getAutomationConfiguration().Driver, MultiaccountPage.class);
		pp.ScrollParking();
