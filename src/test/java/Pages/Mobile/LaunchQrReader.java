...
public class LaunchQrReader {
	...
	public void launchQrReader() throws InterruptedException
	{
		try {
			try {
				...
				((AndroidDriver<MobileElement>)(WebDriver) CreateSession.getAutomationConfiguration().AppiumDriver).startActivity(activity);

			}catch(Exception e) {}

			try {
				ApcoaListeners.logInfo("Clicking  on Accept button");
				CommonUtility.GenericMethods.explicitWait(driver,acceptPushNotificationBtn,20);
			...
			try {
				...
				CommonUtility.GenericMethods.explicitWait(driver,btnSearchLink,10);
				btnSearchLink.click();
	...
	{
		try {
			CommonUtility.GenericMethods.explicitWait(driver,gmptarrifSelection,25);
		...
		CommonUtility.GenericMethods.explicitWait(driver,ParkingTimeTitle,10);
